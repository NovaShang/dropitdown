...
def process(
    ...
    # Filing classifies with the LLM, so it needs a key. Surface a helpful,
    # per-file error (which reaches the macOS notification) rather than dying
    # silently — and point users at the keyless Copy-as-Markdown path.
    if not cfg.api_key:
        msg = ("No API key — add one in Settings → Classification, "
               "or use the Copy as Markdown drop action (no key needed).")
        for f in files:
            emit(watcher.ProcessResult(ok=False, src=f.resolve(), error=msg))
        if not json_out:
...
def undo(record_id: int) -> None:
    """Reverse a record. A normal archive goes back to where it came from; if
    that spot is now occupied it lands in inbox/_review/ instead (its MD note
    is left as an orphan). A "note only" record moved nothing, so undo just
    deletes the orphan note."""
    ...
    cfg = _load_config_or_die()
    # Restore to the original location first; only divert to _review/ if that
    # spot has since been taken (so undo actually reverses the move).
    ok, msg = archive_mod.undo(Path(rec.source_path), Path(rec.archived_path))
    if not ok and "occupied" in msg.lower():
        ok, msg = archive_mod.undo(
            Path(rec.source_path), Path(rec.archived_path),
            restore_dir=cfg.inbox / "_review",
        )
    if ok:
        journal.mark_undone(record_id)
...
_AGENT_SKILL_MARKER = "<!-- generated by `dropitdown agent-skill` -->"

_AGENT_SKILL_TEMPLATE = _AGENT_SKILL_MARKER + """
# Knowledge base — DropItDown vault

This folder is a personal knowledge base built by DropItDown: a Markdown
archive of real-world files (PDFs, screenshots, documents, …). Each note is
the extracted Markdown of one source file, with YAML frontmatter.
...
Each note's frontmatter:
  summary:        one-line description of the file  ← the fast index
  original_file:  file:// URI to the original document
  archived_at:    date the file was archived
  category:       the folder it was filed under

HOW TO ANSWER A QUESTION FROM THIS VAULT:
1. Find candidates fast by grepping the `summary:` lines across *.md here —
   each is a one-line description of an archived file.
2. Read the matching note(s) in full for detail.
3. To open or cite the source document, use its `original_file:` URI.
4. Folder names are the taxonomy — scope a search to a category when you can.

Prefer reading notes over guessing; cite the note path and original_file.
"""


def _render_agent_skill(cfg: Config) -> str:
    cats = []
    if cfg.md_root.exists():
        cats = sorted(
            p.name for p in cfg.md_root.iterdir()
            if p.is_dir() and not p.name.startswith(".")
        )
    tree = "\n".join(f"- {c}/" for c in cats) or "(empty — drop some files first)"
    return _AGENT_SKILL_TEMPLATE.format(md_root=cfg.md_root, tree=tree)


@app.command(name="agent-skill")
def agent_skill(
    write: bool = typer.Option(
        True, "--write/--print",
        help="Write CLAUDE.md into the vault (default), or print to stdout.",
    ),
    force: bool = typer.Option(
        False, "--force", help="Overwrite a hand-written CLAUDE.md."
    ),
) -> None:
    """Teach any agent how to search this vault. Writes a CLAUDE.md into the
    Markdown root so a Claude Code session opened there auto-discovers the
    layout. Use --print to copy the prompt somewhere else instead."""
    cfg = _load_config_or_die()
    content = _render_agent_skill(cfg)

    if not write:
        sys.stdout.write(content)
        return

    dest = cfg.md_root / "CLAUDE.md"
    if dest.exists() and not force:
        existing = dest.read_text(encoding="utf-8", errors="ignore")
        if _AGENT_SKILL_MARKER not in existing:
            console.print(
                f"[yellow]{dest} already exists and wasn't written by DropItDown.[/yellow]\n"
                "Re-run with --force to overwrite, or --print to copy it elsewhere."
            )
            raise typer.Exit(1)
    cfg.md_root.mkdir(parents=True, exist_ok=True)
